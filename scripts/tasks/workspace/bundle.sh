...
xcrun xcodebuild -scheme virtualos -configuration Release -derivedDataPath $DERIVED_DATA_PATH -destination platform=macosx BUILD_LIBRARY_FOR_DISTRIBUTION=YES ARCHS='arm64 x86_64' BUILD_DIR=$XCODE_BUILD_DIR clean build

mkdir -p $BUILD_DIR
cp $XCODE_BUILD_DIR/Release/virtualos $BUILD_DIR/virtualos

(
    cd $BUILD_DIR || exit 1
    zip -q -r --symlinks virtualos.zip virtualos
